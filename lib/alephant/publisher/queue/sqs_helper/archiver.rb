    ...
    module Queue
      module SQSHelper
        class Archiver
          ...
          def async_store(m)
            Thread.new { store(m) }
            logger.metric(:name => "PublisherQueueSQSHelperAsynchronouslyArchiveData", :unit => "Count", :value => 1)
          end

          def store(m)
            logger.metric(:name => "PublisherQueueSQSHelperSynchronouslyArchiveData", :unit => "Count", :value => 1)
